 ...
 * tasks, loading existing files, saving files and quitting. Depending on user
...
public class ProductBacklogGUI extends JFrame implements ActionListener {
	...
	/**
	 * Constructs a {@link ProductBacklogGUI} object that will contain a
	 * {@link JMenuBar} and a JPanel that will hold different possible views of the
	 * data in the BacklogManager.
	 */
	 ...
	 * task items or for quitting the application.
	 */
	 ...
	 * products and associated task lists.
	 * 
	 * @author Dr. Sarah Heckman (heckman@csc.ncsu.edu)
	 */
	private class ProductPanel extends JPanel implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() == comboProductList) {
				...
				try {
					String productName = (String) JOptionPane.showInputDialog(this, "Product Name?",
							"Create New Product", JOptionPane.QUESTION_MESSAGE);
				...
				try {
					String productName = (String) JOptionPane.showInputDialog(this, "Update Product Name?",
							"Edit Product", JOptionPane.QUESTION_MESSAGE, null, null,
							BacklogManager.getInstance().getProductName());
		...
		private class TaskTableModel extends AbstractTableModel {
			...
			/**
			 * Constructs the TaskTableModel by requesting the latest information from the
			 * BacklogManager.
			 */
			 ...
			 * 
			 * @param row is the row's index
			 * @param col is the col's index
	 ...
	 * information about the task item.
	 * 
	 * @author Dr. Sarah Heckman (heckman@csc.ncsu.edu)
	 */
	private class TaskInfoPanel extends JPanel {
		...
		public TaskInfoPanel() {
			...
			scrollNotes = new JScrollPane(txtNotes, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS,
	...
	private class CreateTaskPanel extends JPanel implements ActionListener {
		...
		/**
		 * Creates the JPanel for adding new task to the backlog for the current
		 * product.
		 */
			...
			if (e.getSource() == btnAdd) {
				...
				if (idx < 0 || idx >= types.length) {
					...
					case 1:
						...
						break;
					case 2:
						type = Task.Type.TECHNICAL_WORK;
						break;
					case 3:
						type = Task.Type.KNOWLEDGE_ACQUISITION;
						break;
					default:
						type = null;
					}
